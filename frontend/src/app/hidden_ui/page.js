...
export default function Page() {
          ...
          if (!formInjectedRef.current && chunk.includes(FORM_SENTINEL)) {
            ...
            setMessages((prev) => {
              ...
              clone[assistantIndex] = {
                role: "assistant",
                content: "Let’s set up your x402 gated API real quick. Submit this form!",
              };
  ...
  const regenerate = () => {
    ...
    if (!lastUser) return;
    setMessages((prev) => {
      if (prev[prev.length - 1]?.role === "assistant") return prev.slice(0, -1);
      return prev;
    });
  ...
  const formatContent = (text) => {
    ...
    while (i < lines.length) {
      ...
      if (line.startsWith("# ")) {
        ...
        elements.push(
          <h3 key={i} className="text-lg font-semibold mt-4 mb-2 text-slate-900">
            {line.slice(4)}
          </h3>
        );
        ...
        elements.push(
          <pre key={i} className="bg-slate-900 text-slate-100 rounded-lg p-4 my-3 overflow-x-auto">
  ...
  const Bubble = ({ role, content }) => {
    ...
    return (
      <div className={`flex ${isAssistant ? "justify-start" : "justify-end"} mb-6`}>
        <div className={`flex gap-3 max-w-3xl ${isAssistant ? "flex-row" : "flex-row-reverse"}`}>
          <div className={`flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center ${isAssistant ? "" : "bg-slate-800"}`}>
            {isAssistant ? (
              <img src={gladiusImage.src} className="w-10 h-10 rounded-full" />
            ) : (
          ...
          <div className={`flex-1 px-5 py-3.5 rounded-2xl text-[15px] leading-relaxed ${isAssistantStyle}`}>
            {content ? (
              isAssistant ? (
              ...
              <span className="text-slate-400">Looking into the arena with my sword…</span>
            ) : (
              ""
            )}
          </div>
